...
const CustomerForm = ({ data, onCancel }) => {
    // 1. Multi-field state
    ...
    useEffect(() => {
        if (data) {
            ...
            setFormData({ customer_name: '', phone_number: '', email: '', address: '' });
        }
        setErrors({});
    ...
    // --- API Functions ---
    const saveCustomerApi = async (payload) => {
        const config = {
        ...
        if (data?.customer_id) {
            const res = await api.put(`customers/${data.customer_id}`, payload, config);
            return res.data;
        } else {
            const res = await api.post('customers', payload, config);
            return res.data;
        }
    };
    ...
    const { mutate: saveCustomer, isPending } = useMutation({
        ...
        onSuccess: () => {
            const msg = data ? 'Customer updated successfully' : 'Customer created successfully';
            setTimeout(() => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!formData.customer_name.trim()) {
            newErrors.customer_name = 'The customer name field is required.';
        }
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h2 className="text-xl font-bold text-gray-800 dark:text-white">
                        {data ? 'Update Customer' : 'Add New Customer'}
                    </h2>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                    ...
                    <input
                        ...
                        className={`w-full px-4 py-2 rounded-lg border outline-none transition-all focus:ring-2 bg-white dark:bg-gray-700 dark:text-white dark:placeholder-gray-500 ${errors.customer_name ? 'border-red-400 dark:border-red-500 focus:ring-red-100 dark:focus:ring-red-900' : 'border-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-blue-100 dark:focus:ring-blue-900'
                            }`}
                    />
                    {errors.customer_name && <p className="mt-1 text-xs text-red-500 dark:text-red-400">{errors.customer_name}</p>}
                </div>
